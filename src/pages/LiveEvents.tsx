...
import { getLiveEvents, type LiveEventWithCreator } from '@/lib/supabase/liveEvents';
import { supabase } from '@/lib/supabase/client';

type UiEvent = LiveEventWithCreator & {
  thumbnail: string;
  category: string;
...
function toUiEvent(ev: LiveEventWithCreator): UiEvent {
  const starts = ev.starts_at ?? new Date().toISOString();
  const end = new Date(new Date(starts).getTime() + 2 * 3600000).toISOString();
  return {
    ...ev,
    thumbnail: ev.thumbnail_url ?? '',
    category: '',
    videoUrl: ev.stream_url,
    startTime: starts,
    endTime: end,
    viewerCount: 0,
    hostName: ev.creator?.name ?? 'Host',
    isPremiumOnly: false,
  };
}
...
export function LiveEvents() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await getLiveEvents();
    ...
    <Container>
      <Section>
        ...
        {loading ? (
          ...
          <p className="text-center text-sm text-[#6B7280] py-16">No live events scheduled.</p>
        ) : (
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        fullWidth
                        onClick={() =>
                          locked ? navigate('/pricing') : navigate(`/live-events/${ev.id}/watch`)
                        }
                      >
