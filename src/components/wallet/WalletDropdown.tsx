import { useState, useRef, useEffect } from 'react';
import { Wallet, ChevronDown } from 'lucide-react';
import { useWallet } from '@/context/WalletContext';
import { BuyDoroCoinsModal } from './BuyDoroCoinsModal';
import { timeAgo } from '@/lib/utils';
import { LoadingSpinner } from '@/components/ui/LoadingSpinner';

export function WalletDropdown({ balance: _balance }: { balance: number }) {
  const [open, setOpen] = useState(false);
  const [buyOpen, setBuyOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const { balance, transactions, loading } = useWallet();

  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (ref.current && !ref.current.contains(e.target as Node)) setOpen(false);
    };
    document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, []);

  const recentTx = transactions.slice(0, 5);

  return (
    <>
      <div className="relative" ref={ref}>
        <button
          onClick={() => setOpen(!open)}
          className="flex items-center gap-1.5 bg-[#161618] border border-[rgba(255,255,255,0.08)] rounded-md px-3 h-9 text-sm hover:bg-[#222225] transition-colors"
        >
          <Wallet size={14} className="text-yellow-500" />
          {loading ? <LoadingSpinner size="sm" /> : <span>{balance}</span>}
          <span className="text-[#9CA3AF] text-xs">DC</span>
          <ChevronDown size={14} className="text-[#9CA3AF]" />
        </button>
        {open && (
          <div className="absolute right-0 top-10 w-72 bg-[#1C1C1F] border border-[rgba(255,255,255,0.08)] rounded-lg shadow-xl z-50">
            <div className="p-4 border-b border-[rgba(255,255,255,0.08)]">
              <p className="text-xs text-[#9CA3AF]">Balance</p>
              {loading ? (
                <div className="flex items-center gap-2 pt-1">
                  <LoadingSpinner size="sm" />
                  <span className="text-sm text-[#9CA3AF]">Loading…</span>
                </div>
              ) : (
                <p className="text-2xl font-bold gold-text">{balance} DoroCoins</p>
              )}
            </div>
            <div className="p-3">
              <button
                onClick={() => {
                  setOpen(false);
                  setBuyOpen(true);
                }}
                className="w-full gold-gradient text-black text-sm font-semibold h-9 rounded-md"
              >
                Buy DoroCoins
              </button>
            </div>
            {!loading && recentTx.length === 0 && (
              <div className="border-t border-[rgba(255,255,255,0.08)] px-4 py-3">
                <p className="text-xs text-[#6B7280] text-center">No transactions yet</p>
              </div>
            )}
            {!loading && recentTx.length > 0 && (
              <div className="border-t border-[rgba(255,255,255,0.08)] max-h-40 overflow-y-auto">
                {recentTx.map(t => (
                  <div key={t.id} className="px-4 py-2 border-b border-[rgba(255,255,255,0.05)]">
                    <div className="flex items-center justify-between">
                      <span className="text-xs">{t.description ?? ''}</span>
                      <span
                        className={`text-xs font-medium ${t.type === 'credit' ? 'text-emerald-400' : 'text-red-400'}`}
                      >
                        {t.type === 'credit' ? '+' : '-'}
                        {t.amount}
                      </span>
                    </div>
                    <span className="text-xs text-[#6B7280]">{timeAgo(t.created_at)}</span>
                  </div>
                ))}
              </div>
            )}
          </div>
        )}
      </div>
      <BuyDoroCoinsModal open={buyOpen} onClose={() => setBuyOpen(false)} />
    </>
  );
}
